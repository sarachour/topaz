...
mkdir src/output
rm src/src/nbody_sim.tpz

if [ "$CATEGORY" = "correct" ];
then
	echo "Running: CORRECT"
	cp kernels/correct/nbody_sim.tpz src/src/nbody_sim.tpz
	cd src; make clean &> /dev/null; 
		make &> /dev/null && sutil_run_script.sh run_barn.sh $INPUT $SEED none perfect &> /dev/null
	cd $CDIR
...
cp $SRCDIR/nbody_sim.tpz src/src/nbody_sim.tpz
...
sutil_run_script.sh run_barn.sh $INPUT $SEED $TOPAZ_ARGS $PIN_ARGS $TAGS &> /dev/null  || exit 1
cd $CDIR
