...
extern "C" void get_time(int *ret) /* miliseconds resolution */
{
  double t;
  struct timeval _tp;
...
extern "C" char *extrvalue(char *arg) { /* string of the form "name=value" */
   char *ap;
   ap = arg;
   while (*ap != '\0'){
     if(*ap++ == '=')
...
extern "C" double xrand(double xl, double xh) { return (xl + (xh - xl) * prand()); }
