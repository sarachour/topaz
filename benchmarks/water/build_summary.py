	...
	for dpath in pdata:
		...
		for fdir in listdir(orig_row['path']):
			...
			if(tags[0] == "logs" and orig_row['strategy'] != 'correct'):
				dfile = dpath + "."+fdir;
				row = data[dfile+".0"] = deepcopy(orig_row);
				row['input'] = '.'.join(tags[1:])
				row['strategy'] += ".0";
				...
				row2 = data[dfile+".1"] = deepcopy(orig_row);
				row2['input'] = '.'.join(tags[1:])
				row2['strategy'] += ".1";
				path = row2['path']+"/"+fdir+"/task_summary.1.txt";
				
				fill_task_statistics_summary(row2, path);
