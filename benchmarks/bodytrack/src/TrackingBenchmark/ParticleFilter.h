...
//Types
	...
	float internal_CalcWeights(TrackingModel & model, std::vector<float> & particle, float * tparticle, bool & isvalid, int npart);
protected:
//variables
	TrackingModel *mModel;												//templated model object evaluates particle likelihoods
...
//functions
	...
	virtual void MainCalcWeights(std::vector<Vectorf > &particles);								//calculate particle weights based on model likelihood
...
public:
	...
	ParticleFilter(TrackingModel &model)					{mModel = &model; mMinParticles = 5; mInitialized = false;}; 
...
inline void ParticleFilter::Resample(Vectorf &cdf, Vectorf &bins, Vectorf &samples, int n)
{
	RandomGenerator r;
...
void ParticleFilter::GenerateNewParticles(int k)
{	int p = 0;
...
bool ParticleFilter::Update(fpType timeval)								//weights have already been computed from previous step or initialization
{						
...
{
	estimate.resize(mParticles[0].size());
	for(uint i=0; i < estimate.size(); i++){
		estimate[i] = 0;
