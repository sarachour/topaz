...
void ProjectedCylinder::ImageProjection(const KTCylinder &cyl, Camera &camera)
{
	...
	float norm_r = (float)t_sqrt(rnorm2);
...
void ProjectedCylinder::ImageProjectionTask(KTCylinder * cyl, Camera * camera)
{
	Vector3f point(0.00f,0.00f,cyl->length/2);
	Vector3f r = point - Inverse(cyl->pose) * camera->eye;
	double rnorm2 = r.x * r.x + r.y * r.y;
...
void ProjectedBody::ImageProjectionTask(BodyGeometry * body, Camera * camera)
{
	int n_parts = body->GetBodyPartCount();
	//mProjCyls.resize(n_parts);
	if(mProjCyls.size() < n_parts) mProjCyls.resize(n_parts);
	
	for(int i=0;i<n_parts;i++)
		mProjCyls[i].ImageProjectionTask(body->get(i), camera);
}
...
// Project a body onto multiple cameras
void MultiCameraProjectedBody::ImageProjectionTask(BodyGeometry *body, MultiCamera *cameras)
{
	int n_cameras = cameras->GetCameraCount();
	
	if(mProjBodies.size() < n_cameras) mProjBodies.resize(n_cameras);
	
	for(int i=0;i<n_cameras;i++)
		mProjBodies[i].ImageProjectionTask(body, cameras->get(i));
}
