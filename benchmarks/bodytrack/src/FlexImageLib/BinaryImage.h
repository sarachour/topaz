...
public:
	...
	char * getPrimitive(char * dest){
		//copy over data.
		memcpy(dest, this->mData,(mHeight*mWidth)>>3);
		dest += (mHeight*mWidth)>>3;
		return dest;
	}
	//expand image size.
	char * setPrimitive(char * src, int w, int h){
		mWidth = w;
		mHeight = h;
		Reallocate(w,h); 
		memcpy(this->mData, src,(mHeight*mWidth)>>3);
		src += (mHeight*mWidth)>>3;
		return src;
	}
};
