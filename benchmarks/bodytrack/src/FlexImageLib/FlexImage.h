...
public:
	...
	int getPrimitiveSize(){
		return mSize.width*mSize.height*C;
	}
	char * getPrimitive(char * dest){
		memcpy(dest, mData, mSize.width*mSize.height*C);
		dest += mSize.width*mSize.height;
		return dest;
	}
	char * setPrimitive(char * src, int w, int h, int b){
		mSize.width = w;
		mSize.height = h;
		mBpp = b;
		ReallocateGE(w,h);
		memcpy(mData, src, mSize.width*mSize.height*C);
		src += mSize.width*mSize.height;
		return src;
	}
};
