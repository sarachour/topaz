...
double max(const double f1, const double f2){
  return f1 > f2? f1: f2;
}
...
void simparm::loadParms(const char * file){
  ...
  CNSTNT();
  std::cout << "\nTEMPERATURE                = "<< TEMP << "\n";
  std::cout << "DENSITY                    = "<< RHO << " G/C.C.\n";
  std::cout << "NUMBER OF MOLECULES        = "<< NMOL << "\n";
  std::cout << "TIME STEP                  = "<< TSTEP << " SEC\n";
  std::cout << "ORDER USED TO SOLVE F=MA   = "<< NORDER << "\n";
  std::cout << "NO. OF TIME STEPS          = "<< NSTEP <<"\n";
  std::cout << "FREQUENCY OF DATA SAVING   = "<< NSAVE <<"\n";
  std::cout << "FREQUENCY TO WRITE RST FILE= "<< NRST <<"\n";
  std::cout.flush();
  SYSCNS();
  std::cout << "SPHERICAL CUTOFF RADIUS    = "<< CUTOFF <<" ANGSTROM\n";
  IRST=0;
  fclose(infile);
}
...
void ensemble::INITIA(const char * file){
  ...
  if (random_numbers == NULL) { 
    std::cout << " *** Error: Can't open file random.in (ensemble::INITIA)\n";
    std::cout.flush();
    exit(0);
  }
...
void ensemble::MDMAIN(){
  ...
  std::cout << "RESTART "<< parms->getIRST();
  std::cout << " AFTER "<< parms->getELPST() << " SECONDS\n";
  clearTVIR();
  clearTKIN();

  if(parms->getNSAVE() > 0){
    std::cout << "COLLECTING X AND V DATA AT EVERY %4d TIME STEPS\n";
    std::cout << parms->getNSAVE();
  }
