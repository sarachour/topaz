...
// ----------- THREADING STUFF --------------------------
//
// Variables: _USE_PAR_             -- Uses parallel version of the program
//            _SYNC_INTERF_        -- synchronizes interf calculation
//            _SYNC_POTENG_        -- synchronizes poteng calculation
//            _REPL_ACCUMULATOR_  -- replicates accumulator for poteng and interf
//            _REPL_ACCUMULATOR_POT_  -- replicates accumulator for poteng
//            _REPL_ACCUMULATOR_INT_  -- replicates accumulator for interf
//
//
...
#ifndef _SYNC_INTERF_
#endif
#ifndef _SYNC_POTENG_
#endif
#ifndef _SYNC_VIR_
#endif
// ------------------------------------------------------

#ifdef _REPL_ACCUMULATOR_INT_
#define _REPL_ACCUMULATOR_
#endif

#ifdef  _REPL_ACCUMULATOR_POT_
#define _REPL_ACCUMULATOR_
#endif
...
//    pthread_mutex_t mutex;
  ...
  skratch_pad() { H1pos.vecClr(); Opos.vecClr(); H2pos.vecClr(); VM.vecClr();
    pout = new output_sp;
    pout->H1force.vecClr(); pout->Oforce.vecClr(); pout->H2force.vecClr();

//#ifdef _USE_PARALLEL_
//		pthread_mutex_init(&pout->mutex, 0);
//#endif
  ...
  ~skratch_pad() {
    delete pout;
//#ifdef _USE_PARALLEL_
//     pthread_mutex_destroy(&pout->mutex);
//#endif
  }
  ...
  void storeH1force(double *v) { pout->H1force.vecStore(v); }
  void storeOforce(double *v)  { pout->Oforce.vecStore(v); }
  void storeH2force(double *v) { pout->H2force.vecStore(v); }
...
#ifdef _SYNC_INTERF_
   pout->H1force.vecAddSync(Res[0]);
   pout->Oforce.vecAddSync(Res[1]);
   pout->H2force.vecAddSync(Res[2]);
#else
  pout->H1force.vecAdd(Res[0]);
  pout->Oforce.vecAdd(Res[1]);
  pout->H2force.vecAdd(Res[2]);
#endif
}
...
void h2o::intra_poteng(vector_r *v){
...
void simparm::loadParms(const char * f){
  FILE * infile;
...
private:
   ...
   vector_r POT;
   volatile bool POT_isOriginalValid;
   char * POT_pad;
   vector * POT_replicas;
   char * POT_pad2;
   bool * POT_replicas_init;
   char * POT_pad3;
...
//#define PNUM 1000
//h2o h2os[PNUM];
//skratch_pad spads[PNUM];

ensemble::ensemble(int num){

//   assert( numMol <= PNUM );
//   numMol = num;  molecule = h2os;
//     pad = spads;

  numMol = num;  molecule = new h2o[numMol];
...
void ensemble::INITIA(const char * f){
  vector minimum;
  double t1[3];
  FILE *random_numbers;
...
void potengLoop1Body(int i, void *arg) {
  ensemble *e = (ensemble *) arg;
  e->potengLoop2(i);
}
...
void ensemble::potengLoop1(){
  ...
  get_time(&stop_time);
  cout << "POTENG = " << ((stop_time-start_time)/dticks) << "\n";
///
...
void ensemble::potengLoop2(int idx){
  ...
  for(i=idx+1; i < numMol; i++) {
   ...
   #ifdef _SYNC_POTENG_
    getPOT().vecAddSync(tmp);
   #else
   #ifdef _REPL_ACCUMULATOR_POT_
    getPOT().vecAddRepl(tmp);
   #else
     getPOT().vecAdd(tmp);
   #endif
   #endif
...
/*
void ensemble::inter_poteng2(skratch_pad *p1, skratch_pad *p2){
  double tmp[3];
...
void ensemble::INTRA_POTENG(){
  int i;
  for(i=0; i < numMol; i++){
    molecule[i].intra_poteng(&getPOT());
  }
}
...
void ensemble::CSHIFT2(skratch_pad *p1, skratch_pad *p2, double L[3][15], double *S){
  int i;
  double XAH1[NDIM], XAOx[NDIM], XAH2[NDIM];
  double XBH1[NDIM], XBOx[NDIM], XBH2[NDIM];
  double XMA[NDIM], XMB[NDIM];

  // store into local vectors
  // p1->storeVM(XMA);
  // p2->storeVM(XMB);
  // p1->storeH1pos(XAH1);
  // p1->storeOpos(XAOx);
...
void interfLoop1Body(int i, void *arg) {
  ensemble *e = (ensemble *) arg;
  e->interfLoop2(i);
}
...
void ensemble::interfLoop1(){
  ...
  for(i = 0; i < numMol-1; i++) {
   ...
   cout << "INTERF = " << ((stop_time-start_time)/dticks) << "\n";
...
void ensemble::DUMP(int iter){
  ...
  printf(" FPOT: %6.5f\n",parms->getFPOT());
  printf("  POT: "); getPOT().vecPrint(); printf("\n");
  printf("\n");
  for (i = 0; i < numMol; i++){
    molecule[i].dump();
    printf("\n");
...
#ifdef _OMP_PRINT_
  extern "C" void gomp_main_start();
  extern "C" void gomp_main_finish();
#endif
...
int main(int argc, char **argv) {
  ...
  std::stringstream modeinfo; modeinfo << " Mode: " ;
#ifdef _USE_PARALLEL_
  modeinfo << "UseParallel; ";
#else
  modeinfo << "UseSeq; ";
#endif
  ...
  modeinfo << "SyncVir;" ;
#endif
#if _REPL_ACCUMULATOR_POT_
  modeinfo << "ReplAccumulatorPoteng; ";
#endif
#if _REPL_ACCUMULATOR_INT_
  modeinfo << "ReplAccumulatorInterf; ";
#endif
  ...
  if (argc < 3) {
    printf("Usage: %s input_param_file random_file #threads\n", argv[0]);
    return(1);
  }
  const char * filename = argv[1];
  const char * randomfilename = argv[2];
#ifdef _USE_PARALLEL_
  if (argc >= 4) {
     omp_set_num_threads(atoi(argv[3]));
  } else {
     omp_set_num_threads(atoi(getenv("PL_THREAD_NUM")));
  }
#endif
...
#ifdef _USE_PARALLEL_
  // what was my gomp function for this?
#endif
