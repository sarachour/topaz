...
typedef matrix_par matrix;
//typedef matrix_seq matrix;
...
public:
   ...
   double matGetVal(int row, int col) {
      return val[row][col];
   }
   ...
   void matSub(matrix_seq *q, matrix_seq *r) {
      int i, j;
      for (i = 0; i < NDIM; i++)
         for (j = 0; j < NDIM; j++)
   ...
   void matMult(matrix_seq *q, matrix_seq *r) {
      int i, j, k;
      for (i = 0; i < NDIM; i++)
         for (j = 0; j < NDIM; j++) {
            val[i][j] = 0.0;
   ...
   void matScale(matrix_seq *q, double f) {
      int i, j;
      for (i = 0; i < NDIM; i++)
         for (j = 0; j < NDIM; j++)
...
#else
inline bool isParallelSectionWrapper() {assert(false);}
inline int getThreadNumWrapper() {assert(false);}
inline int getCurrentThreadIdWrapper() {assert(false); }
#endif
...
public:
   ...
   void __updateState() {
// either have reentrant lock, or use bool param passed for synchronized methods...
//      if (isParallelSectionWrapper())
//         pthread_mutex_lock(&acc_mutex);
      assert (!isParallelSectionWrapper());
      for (int k = 0; k < NDIM; ++k) {
...
//// ===================================================================
//// CLASS MATRIX
void* PAD_REG_matrix = 0;
volatile int PAD_REG_IDX_matrix = 0;
pthread_mutex_t PAD_REG_MUTEX_matrix;
volatile bool PAD_REG_INIT_matrix = false;

class matrix_par {

   struct state_val_pad {
      char pad0[PAD_SIZE];
      volatile double val[NDIM][NDIM];
      char pad1[PAD_SIZE];
   };
   ...
   //char pad0[PAD_SIZE];
   volatile bool isConsistent; // is state consistent (isCached)
   volatile double seqVal[NDIM][NDIM];  // this is cached value (cachedVal)
   int vals_id;
   volatile bool isReplInitialized;
   //char pad1[PAD_SIZE];
...
public:
   ...
   static void initPadRegister() {
      if (PAD_REG_INIT_matrix)
         return;
      assert(!isParallelSectionWrapper());
      pthread_mutex_init(&PAD_REG_MUTEX_matrix, 0);
      PAD_REG_matrix = (void*) new state_val_pad*[PAD_REG_SIZE];
      state_val_pad** PAD_REGx = (state_val_pad**) PAD_REG_matrix;
      PAD_REG_IDX_matrix = 0;
      for (int i = 0; i < PAD_REG_SIZE; i++) {
         PAD_REGx[i] = new state_val_pad[MAX_PROC_SIZE];
      }
      PAD_REG_INIT_matrix = true;
   }
   ...
   static void getNextRegisterItem(state_val_pad ** ret_ptr, int & ret_id) {
      ...
      //printf("%d   %d\n", PAD_REG_INIT_matrix, PAD_REG_IDX_matrix);
      //     assert (true == PAD_REG_INIT_matrix);
      //     assert (PAD_REG_IDX_matrix < PAD_REG_SIZE);
      pthread_mutex_lock(&PAD_REG_MUTEX_matrix);
      int id = PAD_REG_IDX_matrix++;
      pthread_mutex_unlock(&PAD_REG_MUTEX_matrix);
      ret_id = id;
      *ret_ptr = ((state_val_pad**) PAD_REG_matrix)[id];

   }

   static void reclaimRegisterItem(int id) {
      assert (id == PAD_REG_IDX_matrix-1);
      --PAD_REG_IDX_matrix;
   }
   ...
   void initReplication() {
      ...
      isReplInitialized = true;
      pthread_mutex_unlock(&acc_repl_mutex);
      printf("MatrixReplicationInit... %p\n", this);
   }
...
//         pthread_mutex_unlock(&acc_mutex);
   ...
   void matSet(matrix_par *m) {
      int i, j;
      if (!isConsistent) __updateState();
      for (i = 0; i < NDIM; i++)
         for (j = 0; j < NDIM; j++)
            seqVal[i][j] = m->matGetVal(i, j);
   }
   ...
   void matTrans() {
      ...
      for (i = 0; i < NDIM; i++)
         for (j = 0; j < NDIM; j++)
            seqVal[i][j] = seqVal[j][i];
   }

   void matAdd(matrix_par *m) {
      int i, j;
      if (!isConsistent) __updateState();
      for (i = 0; i < NDIM; i++)
         for (j = 0; j < NDIM; j++)
   ...
   void matAddSync(matrix_par *m) {
      pthread_mutex_lock(&acc_mutex);
      matAdd(m);
      pthread_mutex_unlock(&acc_mutex);
   }

   void matAddRepl(matrix_par *m) {
      int i, j;
      if (isConsistent) {
         if (!isReplInitialized) initReplication();
         isConsistent = false;
      ...
      for (i = 0; i < NDIM; i++)
         for (j = 0; j < NDIM; j++)
            vals[getCurrentThreadIdWrapper()].val[i][j] += m->matGetVal(i, j);
   }



   void matSub(matrix_par *q, matrix_par *r) {
      int i, j;
      if (!isConsistent) __updateState();

      for (i = 0; i < NDIM; i++)
         for (j = 0; j < NDIM; j++)
            seqVal[i][j] = q->matGetVal(i, j) - r->matGetVal(i, j);
   }

   void matMult(matrix_par *q, matrix_par *r) {
      int i, j, k;
      if (!isConsistent) __updateState();
      ...
      for (i = 0; i < NDIM; i++)
         for (j = 0; j < NDIM; j++) {
            seqVal[i][j] = 0.0;
            for (k = 0; k < NDIM; k++)
               seqVal[i][j] += q->matGetVal(i, k) * r->matGetVal(k, j);
         }
   }

   void matScale(matrix_par *q, double f) {
      int i, j;
      if (!isConsistent) __updateState();

      for (i = 0; i < NDIM; i++)
         for (j = 0; j < NDIM; j++)
            seqVal[i][j] = q->matGetVal(i, j) * f;
   }
