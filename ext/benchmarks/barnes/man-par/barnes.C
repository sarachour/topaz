...
void NBodySystem::updateEnergy(){
  ...
  #pragma omp parallel for private (p, m, velsq) schedule(static)
  for(k = 0; k < nbody; k++) {
    p = bodies->getbody(k);
    matrix m1;
    ...
    tvecLoad(v0,v1);
    tvecScale(v0,m);
    cmphasePos->vecAddRepl(v0);
    tvecLoad(v0,v2);
    tvecScale(v0,m);
    cmphaseVel->vecAddRepl(v0);
   ...
   if (mtot.readval() != 0.0){
      cmphasePos->vecScale(1.0/mtot.readval());
      cmphaseVel->vecScale(1.0/mtot.readval());
   }
}
...
void NBodySystem::stepsystem(parms *p){
  ...
  for (int isteps = 0; isteps < (int) ((stop-now0)/incr + 0.1) ; isteps++) {//while (tnow < stop + 0.1 * incr){
    get_time(&trackstart);
    leaves->LeafSetReset();
    cells->CellSetReset();
...
int parms::getiparam(char *name){
  getparam(name);
  return (atoi(buf));
}

// ---------------------------------------------------------------------------
double parms::getdparam(char *name){
   getparam(name);
   return (atof(buf));
}

void parms::getparam(char *name) {
  int i, leng;
  char *temp;
...
void node::computecofm(){  // <--- Parallelizable...
  int i;
  node *nptr;
  body *p;
  leaf *le;
...
double body::subdiv(node *p, double *res) {
  ...
  drsq = 0.0;
  for (i = 0; i < NDIM; i++) {
    d = p->pos.vecField(i)-pos.vecField(i);
    drsq += d*d;
    res[i] = d;
  }
  return(drsq);
...
void NBodySystem::ComputeAccels(double tol, double eps){
  int i;
  body *p;
  int nbody_local = nbody;

  #pragma omp parallel for schedule(dynamic) private (p)
  for(i=0; i < nbody_local; i++) {
    p = bodies->getbody(i);
