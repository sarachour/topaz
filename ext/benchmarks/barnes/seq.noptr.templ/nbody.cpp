...
void nbody_StartVels(nbody_t * n, double hts){
  int i;
  body_t *body_ptr;

  for(i=0; i < n->nbody; i++) {
	body_ptr = bodyset_get(&n->bodies, bodyptr(i));
    body_startVel(body_ptr, hts);
  }
}

void nbody_ComputeAccels(nbody_t * n, double tol, double eps){
  int i;
  body_t * body_ptr;

  for(i=0; i < n->nbody; i++) {
    body_walksub( bodyptr(i), n->BH_tree, tol, n->size*n->size, eps, n);
  }
}

void nbody_computeForces(nbody_t * n, double tol, double hts, double eps){
  nbody_SwapAccs(n);
  nbody_ComputeAccels(n,tol,eps);
  if(n->nstep > 0){
    nbody_StartVels(n,hts);
  }
  /*
  int i;
  body_t * p;
...
void nbody_stepsystem(nbody_t * n, parms_t *p){
  ...
  while (n->tnow < stop + 0.1 * incr){
    ...
    nbody_computeForces(n,tol,hts,eps);
    get_time(&forcecalcend);
    n->forcecalctime += forcecalcend - forcecalcstart;
	
    get_time(&updatestart);
    nbody_Advance(n, hts,ts);
    
    get_time(&updateend);
    n->updatetime += updateend - updatestart;
    get_time(&trackend);
