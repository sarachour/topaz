...
double back(double point[], double coef[]) {
   ...
   for (jj = 0; jj < NTRAJ; jj++) {
      ...
      sigma = p4 * 4.0e-18 * z165 * engo * engo / (eng + p3 * 1.0e-2 * z133
            * pow(eng, 0.5));
      lamda = lamda_a / sigma;
      alpha = 7.0e-3 / eng;
      ...
      while (icic) {
         ...
         r_over_i = p1 * 100.0 * pow(eng, zex) / zz + zz * zz * zz / eng / p2
               / 1.0e5;
         ...
         step = -lamda * log(ran1);
         c_phi = filtr * (-2 * alpha * ran2 / (alpha - ran2 + 1) + 1) + (1
               - filtr) * (1 - 2 * ran2);
...
main(int argc, char ** argv) {
   ...
   #pragma omp parallel for private (pointt, i, backcoe) schedule (dynamic)
